...
movedat3 <- move(x=vms_move$LONGITUDE,
                y=vms_move$LATITUDE,
                time=vms_move$UTCDATETIM,
                data=vms_move,
...
movedatProj3 <- spTransform(movedat3, CRSobj="+proj=utm +zone=10 +datum=WGS84 +units=m +no_defs +type=crs")
...
```{r}
ptm <- proc.time()
ntrips <- n.indiv(movedatProj3)
#ntrips <- n.indiv(movedat4)
interpolated_data2 <- purrr::map_df(1:ntrips,function(i){

  tmp_movedat <- movedatProj3[[i]]
...
write_rds(vms_regular,here::here('data','processed','matched','interpolation',paste0(process_year,'interpolated_Mary.rds')))
