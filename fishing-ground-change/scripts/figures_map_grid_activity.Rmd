...
- modify plotting function to use set color scale across all maps

- create combined 1-small map (will need to manually set color scale across all grids) 

- created maps of vessel counts
...
1 Small
```{r}
checkvms <- vms_by_grd10_sf[vms_by_grd10_sf$subgroup=="1-small" & !is.na(vms_by_grd10_sf$FID_),]
keep_years.a <- keep_years[which(keep_years %in% checkvms$crab_year)]

myplot10_list.a <- map_fishing_grid10(vms_sf=vms_by_grd10_sf, keep_group="1-small",
                                keep_years=keep_years.a,
                                states_df_coast=states_df_coast,
                                max_vms=NA)
```
```{r}
png(here::here('project-dat','figures',"change_10x10grid_annual_1Small.png"),res=300, height=1500, width=2000)
plot_grid(plotlist=myplot10_list.a,nrow=1,rel_widths=c(2,0.9,0.9,3.4), labels=c("a"))
dev.off()
```
...
2 Small 
```{r}
checkvms <- vms_by_grd10_sf[vms_by_grd10_sf$subgroup=="2-small",]
myplot10_list.b <- map_fishing_grid10(vms_sf=vms_by_grd10_sf, keep_group="2-small",
                                keep_years=keep_years,
                                states_df_coast=states_df_coast,
                                max_vms=2000)
myplot10_list.b <- myplot10_list.b[which(!is.null(myplot10_list.b))]
```
```{r}
png(here::here('project-dat','figures',"change_10x10grid_annual_2Small.png"),res=300, height=1500, width=2500)
plot_grid(plotlist=myplot10_list.b,nrow=1,rel_widths=c(2,0.9,0.9,0.9,0.9,3.3), labels=c("b"))
dev.off()
```
...
```{r}
png(here::here('project-dat','figures',"change_10x10grid_annual_2Large.png"),res=300, height=1500, width=2300)
plot_grid(plotlist=myplot_list.d,nrow=1,rel_widths=c(1.6,0.8,0.8,0.8,0.8,3.3), labels=c("d"))
dev.off()
```
...
```{r}
png(here::here('project-dat','figures',"change_10x10grid_annual_5Large.png"),res=300, height=1500, width=2300)
plot_grid(plotlist=myplot_list.e,nrow=1,rel_widths=c(1.6,0.8,0.8,0.8,0.8,3.3), labels=c("e"))
dev.off()
```


### plot n vms confidential
```{r}
