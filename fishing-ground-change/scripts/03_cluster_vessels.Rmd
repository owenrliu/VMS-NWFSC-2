...
```{r}
lengthdat <- read_csv(here('project-dat','vessel_groups','k4_NaNspeedfilter_2014clusterYR_byArea_nonconTix-group-key.csv')) %>%
  filter(crab_year==2014)


tabledat <- read_csv(here('project-dat','vessel_groups','k4_NaNspeedfilter_2014clusterYR_byArea_noncon-group-key.csv')) %>% filter(crab_year==2014)

tabledat %>% filter(group != 5) %>% group_by(drvid) %>% mutate(occur=n()) %>% filter(occur > 1) %>%
  left_join(lengthdat %>% dplyr::select(drvid,FINAL_LENGTH))
```

remove both vessels from the "small" category and get rid of their 39ft length.
```{r}
edit_vessels <- data.frame(drvid=unique(tabledat %>% filter(group != 5) %>% group_by(drvid) %>% mutate(occur=n()) %>% filter(occur > 1) %>% pull(drvid)),
                           vessel_size=c("small","small"),
                           FINAL_LENGTH=c(39,39))

tabledat %<>% anti_join(edit_vessels)
lengthdat %<>% anti_join(edit_vessels)
```


And filter the VMS data for only 2014 fishing trips.
```{r}
vmsdat_2014 <- filter(vms, crab_year==2014 & drvid %in% tabledat$drvid)
```

```{r}
vmsdat_2014_table <- vmsdat_2014 %>%
  group_by(drvid) %>%
  summarise(ntrips=length(unique(Rec_ID))) %>%
  right_join(tabledat %>% filter(crab_year==2014), by="drvid")
```
which vessels are listed twice 
```{r}
filter(vmsdat_2014_table %>% filter(group != 5) %>% group_by(drvid) %>% summarise(occur=n()), occur > 1)
```

take the median of lengths for vessels that occur in the length data more than once. Because of the fix above, none of these cause vessels to be placed in more than one length category.
```{r}
lengthdat %<>% group_by(drvid) %>% 
  summarise(summary_length=mean(FINAL_LENGTH))
```


```{r}
vmsdat_2014_table %<>% 
  left_join(lengthdat %>% dplyr::select(drvid,summary_length))
```
...
```{r}
vmsdat_2014_table %>%
  group_by(subgroup,group_area,vessel_size) %>%
  summarise(subgroup_vessels=length(unique(drvid)),
            subgroup_trips=sum(ntrips),
            subgroup_length=mean(summary_length),
            subgroup_lengthSD=sd(summary_length))
```
...
total vessels?
```{r}
length(vmsdat_2014_table %>% pull(drvid) %>% unique())
```
