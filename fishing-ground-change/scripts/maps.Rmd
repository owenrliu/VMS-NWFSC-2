...
pg$port_group <- c("Bodega Bay","Fort Bragg","Crescent City","Eureka","Monterey","Morro Bay","SFA")
...
```{r}
  key_filter <- filter(key, size_cat==size)
  uds <- ud_list[[which(names(ud_list) == crab_year)]]
  n_uds <- uds[uds$id %in% filter(key_filter,Region=="North")$group_recode,]
  c_uds <- uds[uds$id %in% filter(key_filter,Region=="Central")$group_recode,]
  
  
  ncol=pnw_palette("Mushroom",n=1)
  ccol=pnw_palette("Shuksan",n=1)
  myplot <- ggplot() +
      geom_polygon(data=states_geo_df, aes(x=long, y=lat, group=group), fill="grey87",linetype=1) +
      geom_point(data=pg_df, aes(x=Lon,y=Lat), color="black") +
      geom_polygon(data=fortify(n_uds), aes(x=long,y=lat, group=group), fill=ncol,color=ncol,alpha=0.3, size=0.5) +
      geom_polygon(data=fortify(c_uds), aes(x=long,y=lat, group=group), fill=ccol,color=ccol,alpha=0.5, size=0.5) +
      geom_text(data=pg_df, aes(x=Lon, y=Lat, label=port_group),size=4, nudge_x=c(110000,100000,120000,65000,85000,70000,45000)) +
      ggtitle(crab_year) +
      theme_void() + theme(legend.position="none",plot.margin=margin(l=-0.5,r=-1,unit="cm"),
                           plot.title = element_text(hjust=0.5)) +
      coord_fixed(xlim=c(350000,800000), ylim=c(3800000,4700000))
...
map_fishing_stacked <- function(north_uds,central_uds,key,keep_years,states_df_coast,label_year=TRUE){
  ...
  new_coast <- states_df_coast
    ...
    geom_point(data=pg_df, aes(x=Lon+j,y=Lat), color="black") +
      ...
      geom_polygon(data=n_uds, aes(x=long,y=lat, group=group), fill=ncol,color=ncol,alpha=0.3, size=0.5) +
      geom_polygon(data=fortify(c_uds), aes(x=long,y=lat, group=group), fill=ccol,color=ccol,alpha=0.3, size=0.5) +
      geom_text(data=pg_df, aes(x=Lon+j, y=Lat, label=port_group),size=5,
  ...
  if(label_year){
    ...
    yrs_labels <- data.frame(yr = seq(as.numeric(first(keep_years)),as.numeric(last(keep_years)))) %>%
      ...
      mutate(xmin=seq(-124, -124+((nyrs-1)*2.5), by=2.5),
             xmax=xmin)
    for(i in seq(1,nyrs)){
      myplot <- myplot +  annotation_custom(grob = textGrob(label = yrs_labels$yr[i], hjust = 0, gp = gpar(cex = 1.2)),
