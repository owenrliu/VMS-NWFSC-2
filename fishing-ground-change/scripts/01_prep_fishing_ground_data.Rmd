...
User Inputs:
```{r}
crab_years <- seq(2011,2014)
states <- c("C")
outdir <- "fishing-ground-change/project-dat/vms"
```
<br>
...
vms.thin %<>%  mutate(to_remove = ifelse(
  (!is.na(in_port) & avg_speed_recalc < 0.75) | (!is.na(in_port) & is.na(avg_speed_recalc)), "in_port",ifelse(
...
vms.filter %>% dplyr::select(Rec_ID, date, ticket_day_of_season, crab_year, port_area) %>%
  ...
  geom_point() + facet_grid(cols=vars(port_area)) + theme(legend.title=element_blank())
```
...
for(y in crab_years){
  ...
  write_rds(vms.out,here::here(outdir,'interpolation_60min',paste0(y,'season_crabfishing.rds')))
}
```
<br>
