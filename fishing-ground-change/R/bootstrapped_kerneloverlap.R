...
bootstrapped_kerneloverlap_proj6= function(split,overlap.method="BA",my_crs=CRS(SRS_string = "EPSG:4326")){
  # matrix with coordinates
  vms_coords <- dplyr::select(analysis(split), LONGITUDE, LATITUDE)
  ...
  slot(vms_sp,'proj4string') <- my_crs
...
TESTbootstrapped_kerneloverlap_proj6 = function(s,my_crs=CRS(SRS_string = "EPSG:4326")){
  # matrix with coordinates
  vms_coords <- dplyr::select(s, LONGITUDE, LATITUDE)
