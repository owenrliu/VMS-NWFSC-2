map_fishing_grid <- function(vms_sf,metric="activity",keep_group,keep_years,states_df_coast,max_vms){
  vms_sf <- vms_sf[vms_sf$subgroup==keep_group & !is.na(vms_sf$GRID5KM_ID),]
    ...
    for(i in seq(1,length(keep_years))){
      ...
      if(i < length(keep_years)){
        ...
        myplot <- ggplot()  +
          ...
          scale_fill_viridis_c(option="magma", direction=-1, name="VMS Density", end=0.9, limits=c(vms_scale_min,vms_scale_max)) +
          ggtitle(yr) +
          theme_void() +
