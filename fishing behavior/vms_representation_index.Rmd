...
fishtix_long %<>% left_join(vms_tickets) %>% 
  mutate(has_vms=replace_na(has_vms,0))
```
...
vessel_size_comp <-  fishtix %>%
  mutate(has_vms=drvid %in% vms_fishtix_vessels) %>% 
...
vessel_size_cpue_plot <- vessel_size_cpue_comp %>% 
  ggplot(aes(ticket_day_of_season,y=mean_prop,ymax=upper,ymin=lower,color=has_vms))+
...
Compare participation using an inverse Simpson index on fisheries

```{r}
vms_fishtix_spp_grps <- fishtix_long %>%
  mutate(has_vms=drvid %in% vms_fishtix_vessels) %>%
  left_join(all_fishtix_spp) %>%
  group_by(drvid,crab_season,has_vms,spp_grp) %>% 
  summarise(tot_rev_grp=sum(tot_revenue_spp)) %>% 
  ungroup()

vms_participation_diversity <- vms_fishtix_spp_grps %>%
  group_by(drvid,has_vms,crab_season) %>% 
  mutate(prop_rev=tot_rev_grp/sum(tot_rev_grp)) %>% 
  summarise(revenue_diversity=1/sum(prop_rev^2)) %>% 
  ungroup() %>% 
  group_by(crab_season,has_vms) %>% 
  summarise(mean_div=mean(revenue_diversity,na.rm=T),
            sd_div=sd(revenue_diversity,na.rm=T)) %>% 
  ungroup()

vms_participation_diversity_plot <- vms_participation_diversity %>% 
  filter(crab_season!="2019-2020") %>% 
  ggplot(aes(crab_season,mean_div,ymax=mean_div+sd_div,ymin=mean_div-sd_div,fill=has_vms))+
  geom_col(position=position_dodge())+
  geom_linerange(position=position_dodge(0.9))+
  ylim(0,2.5)+
  labs(x='Crab Season',fill="Vessel Has VMS",col="Vessel Has VMS",
       y='Revenue Diversity')+
  theme(axis.text.x = element_text(angle=90,vjust=0.5))

vms_participation_diversity_plot
ggsave(here::here('fishing behavior','plots','vms_revenue_diversity_comparison.png'),vms_participation_diversity_plot,w=6,h=4)
```

Join the 3 previous plots

```{r,fig.width=8,fig.height=8}
r1 <- plot_grid(vessel_size_cpue_plot+guides(color='none'),vessel_size_plot,nrow=1,labels=c('a','b'))
vms_comp_plots <- plot_grid(r1,vms_participation_diversity_plot,nrow=2,labels=c('','c'))
vms_comp_plots
ggsave(here::here('fishing behavior','plots','vms_comparisons.png'),vms_comp_plots,w=8,h=8)
```
