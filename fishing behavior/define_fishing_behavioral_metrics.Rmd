...
# key of drvid vs. states
vessel_state_key <- vms %>% 
  group_by(drvid,agency_code) %>% 
  summarise(totland=sum(DCRB_revenue)) %>% 
  ungroup() %>% 
  group_by(drvid) %>% 
  mutate(nstates=n_distinct(agency_code)) %>% 
  # for states with >1 state associated, pick the one with highest landings
  mutate(perc_land=totland/sum(totland)) %>% 
  arrange(desc(perc_land)) %>% 
  slice(1) %>% 
  select(drvid,agency_code)
# save
write_rds(vessel_state_key,here::here('fishing behavior','vessel_state_key.rds'))
```
...
1.  Vessel size (not included as a variable yet)
...
participation_diversity <- fishtix_all %>% 
  filter(drvid %in% vessel_list) %>% 
  # add general indicator of revenue season
  mutate(revenue_season=case_when(
    month(date)>10 ~ paste0(year,"-",year+1),
  ...
  filter(!is.na(revenue_season)) %>% 
  select(-TARGET_lbs) %>% 
  select(drvid,revenue_season,contains('_lbs')) %>% 
  pivot_longer(contains('_lbs'),names_to='fishery',values_to='lbs') %>% 
  group_by(drvid,revenue_season,fishery) %>%  
  summarise(tot_lbs=sum(lbs,na.rm=T)) %>% 
  ungroup() %>% 
  group_by(drvid,revenue_season) %>% 
  mutate(prop_lbs=tot_lbs/sum(tot_lbs)) %>% 
  summarise(landings_diversity=1/sum(prop_lbs^2)) %>% 
  ungroup() %>% 
  rename(crab_season=revenue_season)
...
participation_metrics %>% 
  ggplot(aes(prop_other_tix,landings_diversity))+
  geom_point()+
  geom_smooth(method='lm')+
  labs(x='Proportion of Tickets Targeting Other Fisheries',y='Inverse Simpson Diversity Index of Landings')
# not correlated really
cor(participation_metrics$prop_other_tix,participation_metrics$landings_diversity,use='complete.obs')
```
...
wind_sample <- griddap("nceiPH53sstn1day", # daily Pathfinder winds dataset
