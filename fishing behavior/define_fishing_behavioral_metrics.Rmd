...
Five percent of the vessels made less than $5828 in at least one year of the data. We retain only those vessels that made more than this
...
participation_diversity <- fishtix_long %>%
  filter(drvid %in% vessel_list) %>%
  distinct(Rec_ID,drvid,crab_season,PACFIN_SPECIES_CODE,tot_revenue_spp) %>%
  group_by(drvid,crab_season,PACFIN_SPECIES_CODE) %>%  
...
cor(participation_metrics$prop_other_tix,participation_metrics$revenue_diversity,use='complete.obs')
```

```{r}
# Update 02/09/21
# Checking how much participation diversity is driven by groundfish

# check species prevalence
other_fisheries_revenues <- fishtix_long %>%
  filter(drvid %in% vessel_list) %>%
  distinct(Rec_ID,drvid,crab_season,PACFIN_SPECIES_CODE,tot_revenue_spp) %>%
  group_by(drvid,crab_season,PACFIN_SPECIES_CODE) %>%  
  summarise(tot_rev=sum(tot_revenue_spp,na.rm=T)) %>% 
  ungroup() %>% 
  complete(nesting(drvid,crab_season),PACFIN_SPECIES_CODE,fill=list(tot_rev=0)) %>% 
  ungroup() %>% 
  group_by(drvid,crab_season) %>% 
  mutate(prop_rev=tot_rev/sum(tot_rev)) %>%
  ungroup() %>% 
...
# Across all crab seasons and vessels, this is what the breakdown of mean percent revenue by fishery looks like
other_fisheries_revenues %>% 
