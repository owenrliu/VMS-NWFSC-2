...
dat_clust <- dat_scaled %>% dplyr::select(-(1:2)) %>% NbClust(method="ward.D2",min.nc = 2,max.nc=10,index='all')
```

After clustering, there was a range of number of clusters (groups) suggested by the clustering indices, but the ** majority vote was 4 groups**. 9 of 22 clustering methods suggested that 4 was the best number of clusters. 7 methods suggested 3 groups, 3 suggested 2 groups, and no other group number received more than 2 votes.

```{r}
write_rds(dat_clust,here('fishing behavior','FBT_clusters.rds'))
dat_clust <- read_rds(here('fishing behavior','FBT_clusters.rds'))
```
...
clustnames <- tibble(partition=dat_clust$Best.partition) %>%
  mutate(partition=case_when(
    partition==2 ~ "Roving Specialists",
    partition==3 ~ "Local Generalists",
    partition==4 ~ "Roving Generalists",
    partition==1 ~ "Local Specialists"
  )) %>%
...
cluster3_pca <- fviz_pca(dat.pca,geom.ind="",
...
```{r}
variable_name_matches <- tibble(v=c("n_ports_trip","ports_mean_mth","port_diversity","trip_dur_mean","trip_dur_sd","quant90_day","prop_other_revenue","revenue_diversity","prop_highwinds","entropy_quant90","homerange","vessel_size"),metric=c("Ports per Trip",
         "Ports per Month",
         "Port Diversity",
         "Mean Trip Duration",
         "SD Trip Duration",
...
simplified_groups_plot <- pos %>% ggplot()+
  ...
  labs(x="Mobility, Ports Visited, Fishing Range",y="Participation in Other Fisheries")+
...
dat_rev <- fishtix_plus %>% group_by(crab_season,partition) %>%
  summarise(n = n(),
            ...
            rev_se=rev_sd/n^0.5,
            cost_per_trip = mean(trip_cost,na.rm=T),
            cost_sd=sd(trip_cost,na.rm=T),
...
# Per trip cost +/- 2SE
dat_rev %>% 
  ggplot(aes(crab_season,cost_per_trip,ymin=cost_per_trip-2*cost_se,ymax=cost_per_trip+2*cost_se,group=factor(partition),color=factor(partition)))+
  geom_line(size=1.5)+geom_linerange(size=1.5)+
  labs(x="Crab Season",y="Cost per Trip",title="Per-trip Cost by Behavior Class and Year",color="Group")+
  scale_color_manual(values=viridis_pal(begin=0.2,end=0.8)(4))+
  theme(axis.text.x = element_text(angle=90,vjust=0.5),
        legend.position = 'bottom')
...
heatwave_profit_barplot <- heatwave_profit_summ %>% 
  ...
  annotate('text',label="*",size=8,x=3,y=157)+
  annotate('text',label="*",size=8,x=4,y=225)+
  theme(panel.grid.major=element_blank(),
        panel.grid.minor=element_blank(),
        axis.title=element_text(family="sans",size=14,color="black"),
        legend.position = c(0.2,0.8),
        axis.text.x = element_text(size=12,angle=45,vjust=0.75))+
...
heatwave_non_dcrb_barplot <- mhw_outside_revenue %>%  
  ...
  annotate("text",x=2,y=500,label="*",size=8)+
...
mhw_homeranges_plot <-mhw_homeranges_summ %>% 
  ...
  annotate("text",label="*",x=3,y=4150,size=8)+
  annotate("text",label="*",x=4,y=6550,size=8)+
  theme(panel.grid.major=element_blank(),
        panel.grid.minor=element_blank(),
        legend.position = c(0.2,0.8),
        axis.text.x=element_text(angle=45,vjust=0.75,size=12),
        axis.text.y=element_text(size=12),
        axis.title=element_text(family="sans",size=12,color="black"))
        # legend.position = c(0.2,0.8),
        # axis.text.x = element_text(angle=45,vjust=0.75),)
