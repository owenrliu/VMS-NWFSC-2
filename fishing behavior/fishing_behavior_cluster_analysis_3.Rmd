...
heatwave_profit_barplot <- fishtix_plus %>%
  ...
  scale_color_aaas()+
  geom_col(position=position_dodge(),col='black')+
  geom_errorbar(aes(partition,mean_profit/1000,group=is_heatwave,ymin=(mean_profit-2*se_profit)/1000,ymax=(mean_profit+2*se_profit)/1000),position=position_dodge(0.9),width=0.2)+
  theme(panel.grid.major=element_blank(),
        panel.grid.minor=element_blank(),
        axis.title=element_text(family="sans",size=14,color="black"),
...
heatwave_non_dcrb_barplot <- drvid_dcrb_other %>% 
  mutate(is_heatwave=ifelse(crab_season %in% c('2015-2016','2016-2017','2017-2018'),"MHW","Non-MHW")) %>%
  mutate(is_heatwave=factor(is_heatwave,levels=c("MHW","Non-MHW"))) %>%
  group_by(drvid,crab_season,is_heatwave,partition) %>% 
  summarise(r=sum(tot_other_revenue,na.rm=T)) %>% 
  ungroup() %>% 
  ...
  summarise(n=n(),
            ...
            rev_sd=sd(r,na.rm=T),
            rev_se=rev_sd/n^0.5) %>% 
  ggplot(aes(partition,rev_tot/1000,fill=is_heatwave))+
  scale_color_aaas()+
  geom_col(position=position_dodge(),col='black')+
  geom_errorbar(aes(partition,rev_tot/1000,group=is_heatwave,ymin=(rev_tot-2*rev_se)/1000,ymax=(rev_tot+2*rev_se)/1000),position=position_dodge(0.9),width=0.2)+
  theme(panel.grid.major=element_blank(),
        panel.grid.minor=element_blank(),
        axis.title=element_text(family="sans",size=10,color="black"),
        legend.position = c(0.2,0.8),
        axis.text.x = element_text(angle=45,vjust=0.75),)+
  labs(x="Behavioral Group",y="Non-Dungeness Revenue ($1000s)",fill="Period")
...
footprints_cca <- cca %>%
  select(-data,-cvxhull) %>%
  bind_cols(footprints_cca) %>%
  st_as_sf() %>% 
  mutate(a=st_area(geometry)/1e6 %>% as.numeric())
#
# library(ggforce)
bbox <- st_bbox(footprints_cca)
footprints_cca %>% 
...
mhw_homeranges_plot <-mhw_homeranges %>% 
  ggplot(aes(partition,homerange_area,fill=is_heatwave))+
  scale_color_aaas()+
  geom_col(position=position_dodge(),col='black')+
  geom_errorbar(aes(partition,homerange_area,group=is_heatwave,ymin=(homerange_area-2*se_homerange),ymax=homerange_area+2*se_homerange),position=position_dodge(0.9),width=0.2)+
