...
# Most common transitions by crab season
...
transitions_by_season <- trans_by_season %>% 
...
Use a simple change point analysis to look at change in group membership over time.

```{r}
library(changepoint)
# look for threshold changepoints in number of vessels in each group over time
changepoints_dat <- class_by_yr %>% mutate(lab=str_replace_all(partition," ","_")) %>% 
  select(-partition) %>% 
  pivot_wider(names_from='lab',values_from = 'n_vessels')

# for Local Generalists group
cp <- cpt.mean(changepoints_dat[,-1] %>% as.matrix() %>% t(),penalty="SIC",class=F)
cp1<-cpt.mean(changepoints_dat$Local_Generalists,penalty="SIC")
cp2<-cpt.mean(changepoints_dat$Local_Specialists,penalty="SIC")
cp3<-cpt.mean(changepoints_dat$Roving_Generalists,penalty="SIC")
cp4<-cpt.mean(changepoints_dat$Roving_Specialists,penalty="SIC")
```
...
metrics_by_partition_week %>% 
  ggplot(aes(week,mean_profit_per_trip,col=partition,fill=partition))+
  geom_point(size=0.5)+
  geom_smooth()+
  ylim(NA,40000)+
  # geom_pointrange(aes(ymin=lower,ymax=upper,),size=0.25)+
  labs(x="Week of Season",y="Profit per Trip",col="Group",fill="Group")+
  scale_color_manual(values=viridis_pal(begin=0.2,end=0.8)(4))+
  scale_fill_manual(values=viridis_pal(begin=0.2,end=0.8)(4))
...
tot_profit_season_plot <- total_rev_landings_season %>%
  ...
  ggplot(aes(crab_season,profit_tot,ymin=profit_tot-2*profit_se,ymax=profit_tot+2*profit_se,
             group=factor(partition),color=factor(partition)))+
  geom_line(size=1.5,position=position_dodge(width=0.2))+geom_linerange(size=1,position=position_dodge(width=0.2))+
  labs(x="Crab Season",y="Mean Total Season Profit ($1000s)",title="",color="Group")+
  scale_color_manual(values=viridis_pal(begin=0.2,end=0.8)(4))+
  theme(axis.text.x = element_text(angle=90,vjust=0.5),
        legend.position = 'bottom')

ggsave(here::here('fishing behavior','total_revenue_by_season.png'),tot_profit_season_plot,w=8,h=6)
...
other_revenue_season <- drvid_dcrb_other %>% 
  ...
  ungroup() %>% 
  group_by(crab_season,partition) %>% 
  summarise(mean_dcrb=mean(tot_dcrb,na.rm=T),mean_other=mean(tot_other,na.rm=T),mean_tot=mean(tot_rev,na.rm=T)) %>% 
  ungroup() %>% 
...
outside_revenue_by_year <- other_revenue_season %>% 
  ...
  theme(axis.text.x = element_text(angle=90,vjust=0.5),
        legend.position = 'bottom')
ggsave(here::here('fishing behavior','outside_revenue_by_season.png'),outside_revenue_by_year,w=8,h=6)
```
