...
process_year <- seq(2016,2019)
plot_output <- TRUE
```


***
## Existing Vessel Length Key

If this script was run previously, just match landings to vessel lengths.

```{r, eval=FALSE}
for(y in process_year){
  # read in landings rds file
  landings_subset <- read_rds(here::here('data','processed','fish tickets compFT',paste0(y,'fishtix.rds')))
  
  # read in vessel lengths rds file
  vessel_length_key_df_subset <- read_rds(here::here('data','processed','vessel length keys',paste0("vessel_length_key_", y, ".rds")))
  
  # join
  dcrb_landings_length <- left_join(landings_subset,vessel_length_key_df_subset, by=c("drvid", "agency_code","year"))
  
  # write
  write_rds(dcrb_landings_length,here::here('data','processed','fish tickets compFT',paste0(y, "fishtix_vlengths.rds")))
  
  # save length key for plotting?
  if(plot_output){
    if(y==process_year[1]){
      vessel_length_key <- vessel_length_key_df_subset %>% mutate(year=y)} else{vessel_length_key %<>% bind_rows(vessel_length_key_df_subset %>% mutate(year=y))}
  }
   
   # print out summary of missing data for year
    missing <- with(vessel_length_key_df_subset, table(agency_code,is.na(FINAL_LENGTH)))
    cat("Vessels Missing Lengths for ", y,"\n")
    print(missing)
}
```
