...
```{r read data}
vms_raw <- read_csv(here::here('data','raw','vms',paste0("VMS_all_data_", year, ".csv")),col_types='cddcddcc')
...
vms <- vms_raw %>% 
  ungroup() %>% 
  mutate(VMS_RECNO=row_number()+year*1e7) %>% 
...
write_rds(vms_dupes, here::here('data','processed','vms',paste0(year, "_duplicates_only.rds")))
```
...
ggplot()+
  ...
  labs(x='Longitude',y='Latitude',title=paste0(year," VMS records"))
```
...
```{r save rds}
write_rds(vms_ordered,here::here('data','processed','vms',paste0(year,'_vms_clean.rds')))
```
...
# write
library(foreign)
write.dbf(dataframe=vms_dbf, here::here('data','processed','vms',paste0(year,'_vms_clean.dbf')), factor2char=TRUE)
```

```{r}
x<-proc.time()-alltime
cat('This script for',year,'VMS data took',x[3]/60,'minutes to run.')
```
