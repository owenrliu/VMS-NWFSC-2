...
dat_targets_other %<>%
  ...
  mutate(date=as.Date(date,tryFormats=c("%m/%d/%y","%d-%b-%y"))) %>% 
  ##
...
if(any(is.na(dat_targets_other$date))){
  message("WARNING: ",sum(is.na(dat_targets_other$date))," dates processed incorrectly. check format for as.Date function.")
}
```
<br>
