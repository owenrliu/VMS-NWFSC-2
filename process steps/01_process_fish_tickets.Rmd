...
rawdat   <- read_csv(here::here('data','raw','fish tickets 2016-2022 010323.csv'))
...
```{r}
colnames(rawdat)[(problems(rawdat) %>% select(col) %>% distinct())$col]
# The columns with parsing errors are usually from columns that we do not need anyway
```
<br>

In the 2016-2022 data, it looks like read_csv is trying to read in TRIP_ID as a factor instead of numeric.
<br>
...
## add into to column selection AFI_EXVESSEL_REVENUE - already adjusted for inflation **REMOVED FOR pre-2016 data for CHANGING FISHING GROUNDS PROJECT 2023-01-21**
## add into column selection FTID: Source agency fish ticket identifier. This number is unique within a year and agency, but not necessarily across years or agencies.
...
```{r dates}
dat_targets_other %<>%
  ## edited for 2016-2022 tickets
  mutate(date=lubridate::parse_date_time(date,c("mdy_HMS","dmy_HMS")))
  ##
dat_targets_other %<>%
...
```{r}
## edited for 2016-2022 tickets
if(any(is.na(dat_targets_other$date))){
  message("WARNING: ",sum(is.na(dat_targets_other$date))," dates processed incorrectly. check format for parse_date_time function.")
}
```
<br>
...
Clean up the output by re-ording the columns of interest
