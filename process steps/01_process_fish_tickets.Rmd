...
We need FTID; what is the error?
```{r}
probs <- problems(rawdat) %>% filter(col=="FTID") %>% distinct(); probs
message("of the ",dim(rawdat)[1], " rows in the dataset, ", length(probs$row), " rows have a FTID error.")
```
<br>
...
## add into to column selection AFI_EXVESSEL_REVENUE - already adjusted for inflation **REMOVED FOR CHANGING FISHING GROUNDS PROJECT 2023-01-21**
...
for(y in 1:length(yrs_of_data)) {
 dat_out %>% 
    ...
    write_rds(here::here('data','processed','fish tickets compFT',paste0(yrs_of_data[y],"fishtix",".rds",collapse=""))) 
}
