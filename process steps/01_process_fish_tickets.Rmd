...
```{r rawdata}
rawdat <- read_csv(here::here('data','raw','fish tickets','2006_2016_compFT.csv'))
rawdat <- rawdat %>%
...
```{r}
rawdat.sub.filter %>% count(Rec_ID) %>% 
  ggplot(aes(n,..count..))+
...
```{r}
rawdat.w.targets <- rawdat.sub.filter %>% 
  distinct() %>% 
...
recID_attributes <- rawdat.sub.filter %>% 
...
dat_out %<>%
  ...
  select(Rec_ID,pacfin_port_code,port_group_code,drvid,agency_code,removal_type_code,removal_type_name,gear_code_all,TRAP.OR.POT.OR.RING,date,month,year,year_mo,jdate,year_jdate,Week,year_Wk,contains("_lbs"),contains("_rev"),species_code_all,other_lbs,other_rev) %>% 
...
```{r save}
write_rds(dat_out,here::here('data','processed',paste0(years,"fishtix",".rds",collapse="")))
