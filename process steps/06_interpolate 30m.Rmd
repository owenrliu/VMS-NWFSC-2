...
In this step, we produce a version of the VMS data that is interpolated such that records are evenly distributed every **30 minutes**.
...
vms_gapfiltered <- vms_filtered %>%
  ...
  mutate(keep_remove_timelag=ifelse(max(segment_dur,na.rm=T) > (4*3600) | (segment_dur/trip_dur) > max_gap_ptrip, "remove", "keep")) %>% 
...
With these criteria, out of `r ntottrips` unique trips in the filtered dataset, we flagged `r nremovedtrips` for removal (`r (nremovedtrips/ntottrips)*100`%), with `r nremovedlastseg` of these because of the last segment in the trip. For now, we instead retain these `r nremovedlastseg` trips for investigation later.
...
interpolated_data <- purrr::map_df(1:ntrips,function(i){
  ...
  inter_movedat <- interpolateTime(x=tmp_movedat, time=as.difftime(30, units="mins"), spaceMethod='euclidean')
...
write_rds(vms_regular,here::here('data','processed','matched','interpolation_30min',paste0(process_year,'interpolated.rds')))
```
