...
#This section may provide a message "no non-missing arguments to max; returning -Inf" which is a warning, and not an error
```{r}
...
#The code for creating a 'move' object and for runing interpolation have been updated due to changes in GDAL and PROJ libraries.
#The projection command in the following chunk has been updated to use proj6 string fromatting. To check your GDAL and PROJ versions use code rgdal::rgdal_extSoftVersion()
```{r move}
<<<<<<< HEAD
...
movedat <- move(x=vms_move$X_COORD,
                ...
                data=vms_move,
                proj=sp::CRS("+proj=utm +north +zone=10 +ellps=WGS84 +datum=WGS84"),
...
start.time <- Sys.time()
movedat <- move(x=vms_move$X_COORD,#[1:100],
                y=vms_move$Y_COORD,#[1:100],
                time=vms_move$UTCDATETIM,#[1:100],
                data=vms_move[1:100,],
                proj=sp::CRS("+proj=utm +zone=10 +datum=WGS84 +units=m +no_defs +type=crs"),
                animal=vms_move$Rec_ID#[1:100]
                )
Sys.time() - start.time
...
#                animal=vms_move$Rec_ID)
#Sys.time() - start.time
#The old code gives the following error message: Error in SpatialPointsDataFrame(coords = cbind(x, y, deparse.level = 0),  :   row.names of data and coords do not match
...
>>>>>>> a2419199c5e836f0084bc35c0b60b0e94e2844dc
```
...
```{r}
ptm <- proc.time()
#ntrips <- n.indiv(movedat) #OLD CODE
ntrips <- n.indiv(movedatProj) #NEW CODE
interpolated_data <- purrr::map_df(1:ntrips,function(i){

  # tmp_movedat <- movedat[[i]] #OLD CODE
  tmp_movedat <- movedatProj[[i]] #NEW CODE
...
#NEW CODE
#join datasets using original UTCDATETIM, X_COORD and Y_COORD columns prior to replacing them with the results from the interpolation
orig_vms <- interpolated_data %>%
  filter(sensor=="unknown") %>% 
  left_join(vms_move,by = c("Rec_ID", "drvid", "UTCDATETIM", "X_COORD", "Y_COORD")) %>% 
  dplyr::select(-UTCDATETIM,-X_COORD,-Y_COORD) %>% 
...
#This chunk currently gives the following message: "A new CRS was assigned to an object with an existing CRS:+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0 without reprojecting. For reprojection, use function spTransform"
```{r}
# tm <- proc.time()
vms_sp <- vms_regular
coordinates(vms_sp) <- c("LONGITUDE", "LATITUDE") 
proj4string(vms_sp)<-proj4string(bathy.grid) <- CRS("+init=epsg:4326") # WGS 84
bathy.points <- over(vms_sp, bathy.grid)$vms_composite_bath/10
# proc.time()-tm
