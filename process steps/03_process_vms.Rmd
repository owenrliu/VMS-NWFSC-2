...
## 2021 has different UTCDATETIM format to previous years
if (process_year == 2021){
    vms_raw %<>%
  mutate(UTCDATETIM=parse_date_time(UTCDATETIM, c("mdy HMS")))
} else {
    vms_raw %<>%
...
Get bathymetry at VMS data points. Note that the bathymetry layer is in decimeters, so we need to divide by 10 to get depth in meters.
