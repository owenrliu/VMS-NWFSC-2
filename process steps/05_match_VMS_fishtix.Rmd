...
fishtix %<>%
  ...
  # find the ticket BEFORE each ticket and note its time. Instead of creating NAs for the first ticket of the year for each vessel, assign a default value of January 1st. (CAN CHANGE THIS LATER, but for now allows us to use the first ticket of the year for each vessel)
  mutate(prev_ticket_dttm=lag(end,1,default=ymd_hms(paste0(process_year,"-01-01", " 23:59:59"),tz= "America/Los_Angeles"))) %>%
...
# some checks
x <- fishtix_vms %>% filter(end<="2012-01-01 23:59:59 PST")
...
write_rds(fishtix_vms_all,here::here('data','processed','matched',paste0(process_year,'matched_all.rds')))
write_rds(fishtix_vms_only,here::here('data','processed','matched',paste0(process_year,'matched_vms_only.rds')))
```
