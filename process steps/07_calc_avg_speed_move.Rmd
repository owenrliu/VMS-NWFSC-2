...
First, the cleaned VMS data from the previous step

```{r}
vms <- read_rds(here::here('data','processed','matched',paste0(process_year,'matched_port_dist_filter.rds')))
```

## Calculate Speeds

Convert to `sf` object, using the UTM-converted units

```{r}
vms_sf <- vms %>% 
  st_as_sf(coords=c('X_COORD','Y_COORD'),crs = "+proj=utm +north +zone=10 +ellps=WGS84")

# Calculate distances by grouping by Vessel then lagging observations by one
...
vms_speed_comp_plot1 <- ggplot(vmsdat_speed_comparison, aes(y=diff.speed, x = Port_Of_Landing)) +
  geom_boxplot() + 
  ylab("m/s") +
  ggtitle("Difference between Recorded & Calculated Average Speed.") +
  theme(axis.text.x = element_text(angle=90, hjust=1))
vms_speed_comp_plot1
ggsave(paste0(vmsdir, "vms_speed_comp.png"),vms_speed_comp_plot1)

vms_speed_comp_plot2 <- ggplot(vmsdat_speed_comparison, aes(y=AVG_SPEED, x = avg_speed_ms)) +
  geom_point() + geom_smooth(method='lm',se=F)+
  geom_abline(intercept=0,slope=1)+
  labs(y="Recorded",x="Calculated",title="Recorded vs. Calculated Speeds")
vms_speed_comp_plot2
ggsave(paste0(vmsdir, "vms_speed_comp2.png"),vms_speed_comp_plot2)

fivenum(vmsdat_speed_comparison$diff.speed)
```
<br>
```{r}
...
vmsdat_ext <- vmsdat_ext %>%
  mutate(avg_speed_ms = coalesce(avg_speed_ms, AVG_SPEED))
```
<br>
...
for(i in seq(1, length(years))){
  ...
  write_csv(tmp_out, paste0(vmsdir,"VMS_Outputs_wTARGET_10d_lookback_", years[i],"_speed_cleaned.csv"))
}
```
<br>
...
Removed records
```{r}
write_csv(vmsdat_removed, paste0(vmsdir,"VMS_Outputs_wTARGET_10d_lookback_", years[1], "_speed_cleaned_HighSpeedRemoved.csv"))
```
