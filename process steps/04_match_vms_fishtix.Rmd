...
Path adjustment to confidential data location
```{r}
...
```{r load tix}
fishtix <- read_rds(here::here(cdir,'data','processed','fish tickets',paste0(process_year,'fishtix.rds')))

if(process_year>2009){
  fishtix_prev <- read_rds(here::here(cdir,'data','processed','fish tickets',paste0(process_year-1,'fishtix.rds'))) %>% 
...
```{r load vms}
vms <- read_rds(here::here(cdir,'data','processed','vms',paste0(process_year,'_vms_clean.rds')))

if(process_year>2009){
  vms_prev <- read_rds(here::here(cdir,'data','processed','vms',paste0(process_year-1,'_vms_clean.rds'))) %>% 
...
# How long are trips?
fishtix_vms %>% 
  dplyr::select(Rec_ID,trip_dur,has_vms) %>%
  mutate(trip_dur=as.integer(trip_dur)) %>% 
  distinct() %>% 
  ungroup() %>% 
  mutate(has_vms=ifelse(has_vms,"VMS Trips","Non-VMS Trips")) %>% 
...
fishtix_vms %>% 
  dplyr::select(Rec_ID,multispecies,has_vms) %>%
  distinct() %>% 
  mutate(has_vms=ifelse(has_vms,"VMS Trips","Non-VMS Trips")) %>% 
...
```{r eval=FALSE}
fishtix_vms %<>% dplyr::select(-month)
fishtix_vms_all <- fishtix_vms
fishtix_vms_only <- fishtix_vms %>% filter(has_vms==1)
write_rds(fishtix_vms_all,here::here(cdir,'data','processed','matched','matching',paste0(process_year,'matched_alltix.rds')))
write_rds(fishtix_vms_only,here::here(cdir,'data','processed','matched','matching',paste0(process_year,'matched_vmstix_only.rds')))
```
