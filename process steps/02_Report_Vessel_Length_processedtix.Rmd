...
We use a decision tree for calculating vessel length from PacFIN data. The template allows for one year or multiple years of fish tickets to be run at the same time. The default option is to determine vessel lengths for Dungeness crab vessels ONLY. To run the vessel length calculations for *all* vessels in the landings data, set option `dcrb_only` to `FALSE`.
...
#### Read in Fish Ticket Data and Filter to Vessels of Interest
