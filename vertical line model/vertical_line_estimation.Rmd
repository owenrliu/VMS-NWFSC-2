...
vms_sf <- vms_sf %>% 
  ...
  left_join(ORtier,by=c('year','monthnum','drvid'))
```
...
It took `r round(x[3]/60,2)` minutes to calculate traps for all trips. (5.4 hrs on 4 cores)
...
traplocs <- read_rds(here::here('vertical line model',"sim_traps_speed_depth_filters.rds")) %>% ungroup()
