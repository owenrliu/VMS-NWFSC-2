...
BIA_sf <- sf::st_read('~/Documents/RAIMBOW/Processed Data/Samhouri et al. whales risk/Input_Data/BIAs/CetMap_BIA_WGS84/CetMap_BIA_WGS84.shp') %>%
  st_transform(st_crs(grd))
  # convo with OL 030920: add st_transform(st_crs(vms)) to put BIAs in same projection
...
### i still haven't managed to make this work the way i would like
...
ltys <- c('Region'=2)
cols <- c('Humpback BIA'='firebrick1')
#cols <- c('Humpback BIA'='firebrick1', 'Blue BIA' ='lightblue')
...
dcrb_nov_mar_plot <- ggplot()+
  ...
  #geom_sf(data=BIA_bm,fill=NA,aes(col='Blue BIA'),size=1)+
  #geom_sf(data=states,col=NA,fill='gray60')+
